 ...
 *
 * Split out of firmware/main/control.h so both firmwares share one
 * definition: the rest of that header depends on esp_err.h and on
 * ESP-specific handler types, but these two enums depend on nothing.
 *
 * Compiled into BOTH firmwares -- the ESP-IDF build under firmware/main and
 * the Zephyr build for the nRF54L15 -- so it must gain no SDK includes. A
 * board need not implement every command; it replies
 * SN_STATUS_UNKNOWN_COMMAND for those it does not. What must never differ is
 * the numbers.
 */
